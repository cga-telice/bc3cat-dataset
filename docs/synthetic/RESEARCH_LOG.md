...
- **6 prompt echoes** — all 6 candidates of `TEXTO template (OEB010$)`: the `new` text contains the prompt scaffold itself (`Campo destino: TEXTO Plantilla actual: …`). These are not paraphrases and must be **rejected (all 6)** in review; they are the only known case of the model echoing the prompt into a candidate.
